...
setInterval(() => {
  http.get(`http:shadybob.glitch.me/`);
...
const youtube = new YouTube("AIzaSyChx1xksHRy1C-M5YdbJ3eJwUbTvNHSUSA
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@454828705630519326>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "السلام عليكم") {
    message.channel.send("**:heart:وعليكم السلام ورحمه الله وبركاته:heart:**");
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setby")) {
    ...
    client.on("guildMemberAdd", member => {
      ...
      var channel = member.guild.channels.find(gg => gg.name === "log-server"); //// تعديل اساسي
      if (!channel) return;
...
client.on("message", message => {
  if (message.content.split(" ")[0] === prefix + "لينك") {
    message.channel
      .createInvite({
        thing: true,
        maxUses: 5,
